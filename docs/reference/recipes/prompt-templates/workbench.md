...
1. Identify what kind of upload this is.
...
2. Confirm whether WorkBench is the right tool.
...
4. Confirm required fields and relationships.
...
5. Import the small test batch into WorkBench.
...
8. Check matching and duplicate behavior.
...
9. Upload only the test batch first.
...
10. Save or export the upload plan if it will be reused.
...
12. If validation or upload gets stuck, check the Worker.
...
Help me solve the following Specify 7 WorkBench upload problem:
    [Describe the WorkBench upload problem here]

Context:

- User role or permissions: [Choose one or more: admin / collection manager / read-only / unknown]

- Area affected: [Choose one or more: WorkBench / data entry / batch edit / Collection Object / Collecting Event / Locality / Geography tree / Taxon tree / Determination / Preparation / Agent / Storage / attachments / queries / forms / reports and labels / API / server or configuration / security and accounts / Darwin Core export / GBIF publishing / public portal / other]

- Upload operation: [Choose one or more: import dataset / map columns / validate dataset / resolve matching / upload new records / update existing records / upload attachments / upload taxonomy / upload localities / upload collecting events / upload collection objects / upload determinations / upload preparations / save upload plan / reuse upload plan / other]

- Batch status: [Choose one: planning only / test batch / full dataset / already uploaded / upload failed / validation stuck / unknown]

- Dataset details:

    - [File type: CSV / XLSX / TSV / other]
    - [Approximate row count]
    - [Approximate column count]
    - [Was a small test batch used? yes / no / unknown]
    - [Target collection or discipline]
    - [Target table or main record type]
    - [Does the upload create new records? yes / no / unknown]
    - [Does the upload update or match existing records? yes / no / unknown]
    - [Does the upload touch shared records such as Localities, Agents, Taxa, Geography, or Collecting Events? yes / no / unknown]

- Mapping details:

    - [Were columns mapped manually, with AutoMapper, or both?]
    - [Was an upload plan used? yes / no / unknown]
    - [Was the upload plan reused from another dataset? yes / no / unknown]
    - [Which fields/tables are mapped?]
    - [Any columns intentionally not mapped?]

- Validation details:

    - [Validation status]
    - [Number or type of invalid cells, if known]
    - [Matching ambiguity messages, if any]
    - [Pick list, formatter, required field, date, coordinate, or relationship errors, if any]
...
- Evidence: [Screenshot of WorkBench grid, mapping, validation panel, upload result, error message, source-file sample, upload plan JSON, logs, API response, query result, or affected records]

Please answer with:

1. The most likely cause.
2. Whether this is primarily a source-data issue, mapping issue, validation issue, matching/duplicate issue, permissions issue, Worker/server issue, schema/configuration issue, or downstream export/display issue.
3. The correct Specify 7 process for this kind of WorkBench upload.
4. Step-by-step troubleshooting or fix instructions.
5. A recommended small-batch testing approach before any full upload.
6. Relevant Beaty, Specify documentation, forum, GitHub issue, Darwin Core, or GBIF references.
7. Warnings about actions that could affect data integrity, permissions, shared records, Collection Objects, Collecting Events, Localities, Geography, Taxa, Determinations, Preparations, Agents, attachments, reports, labels, exports, public portals, or production systems.
