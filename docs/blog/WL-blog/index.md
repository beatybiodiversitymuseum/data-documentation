...
Getting a runconfig.yaml that was generated by slim, easily moPopulated collection_schema_map in config.yaml mapping integer collection IDs to schema names for all 13 Beaty collections (algae, vascular plants, bryophytes, lichen, fungi, birds, herpetology, mammals, marine invertebrates, fishes, paleontology, entomology, teaching & learning)
Moved collection ID and query ID resolution out of hardcoded config into .env variables via collection_config.py helpers (resolve_collection_id, resolve_query_id)
Refactored report.py to accept config as a parameter rather than reading constants from module-level globals, making it collection-agnostic
Saved query field translation: discovered that Specify's spquery API endpoint (/api/specify/spquery/{id}/) returns a fields array with stringid, position, isdisplay, and (always null) columnalias. Wrote stringid_to_api_name() to translate the dotted stringid path into the __-joined API field name the ?fields= parameter expects
Field classification: query fields are classified into three categories at generation time — join outputs (produced by cache joins, never fetched directly), temporaries (isdisplay=False, fetched for pipeline use and dropped before CSV), and direct display fields
Staleness-based regeneration: is_runconfig_stale() is a pure function that compares the mtime of runconfig.yaml against the mtimes of config.yaml and all names files; regeneration is skipped if the output is current
Per-collection sessions: each collection requires its own Specify login because saved queries are scoped to their owner collection — a session authenticated for collection A cannot read collection B's query
Pure/impure separation in generator: all translation and merging logic (stringid parsing, field classification, schema assembly) is in pure functions; only the login, HTTP fetch, and file write live in the impure shell generate_runconfig()

### Resolved Problems

columnalias is always null in Specify — the saved query cannot provide human column names. Solved by introducing <name>-names.yaml files as the human-authored stringid → output name mapping, keeping them separate from the generated runconfig.yaml
404 on cross-collection query fetch — fetching a query from collection B while authenticated as collection A returns a 404. Solved by logging in once per collection during generation and discarding the session afterwards
